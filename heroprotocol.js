...
const protocol29406 = exports.protocol =  require('./node_modules/heroprotocol/lib/protocol29406');

const version = exports.version = require('./node_modules/heroprotocol/package.json').version;
...
exports.open = function (file, noCache) {
  ...
  if (!lastUsed || !(lastUsed instanceof MPQArchive) || file !== lastUsed.filename || noCache) {
    ...
    // parse header
    archive.data = {};
    header = parseStrings(protocol29406.decodeReplayHeader(archive.header.userDataHeader.content));
    // The header's baseBuild determines which protocol to use
    archive.baseBuild = header.m_version.m_baseBuild;

    try {
      archive.protocol = require(`./node_modules/heroprotocol/lib/protocol${archive.baseBuild}`);
    } catch (err) {
      archive.error = err;
    }
...
exports.parseFile = function (filename, buffer, build) {
  ...
  try {
    protocol = require(`./node_modules/heroprotocol/lib/protocol${build}`);
  } catch (err) {
    return undefined;
  }
